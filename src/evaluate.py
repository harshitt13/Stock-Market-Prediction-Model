...
# ---------------------------------------------------------------------------
# DEPRECATED price-space API, removed in Phase 4
# ---------------------------------------------------------------------------
# These are the old definitions, kept verbatim only so that the not-yet-ported
# model modules keep importing successfully between commits. Do not call them
# from new code: derive_direction_labels differences the prediction *series*,
# which is the fold-boundary bug section 4.1 exists to remove.
...
def derive_direction_labels(prices: np.ndarray, threshold: float = DIRECTION_THRESHOLD):
    """DEPRECATED. Use :func:`direction_labels` on returns instead."""
...
def compute_classification_metrics(
    y_true_prices, y_pred_prices, model_name: str = "Model",
    threshold: float = DIRECTION_THRESHOLD,
) -> Dict[str, Any]:
    """DEPRECATED. Use :func:`directional_accuracy` in return space."""
    ...
    pred_labels_aligned[pred_labels_aligned == -1] = 0
    ...
    return {
        "Model": model_name,
        "Directional Accuracy (%)": round(accuracy_score(a, p) * 100, 2),
        "F1 Score": round(f1_score(a, p, average="macro", zero_division=0), 4),
        "Precision": round(precision_score(a, p, average="macro", zero_division=0), 4),
        "Recall": round(recall_score(a, p, average="macro", zero_division=0), 4),
        "Confusion Matrix": confusion_matrix(a, p, labels=[0, 1]),
    }


def compute_regression_metrics(y_true, y_pred, model_name: str = "Model") -> Dict[str, Any]:
    """DEPRECATED. Use :func:`return_error_metrics` and :func:`r2_oos`."""
...
def compute_metrics(y_true, y_pred, model_name: str = "Model") -> Dict[str, Any]:
    """DEPRECATED. Use :func:`evaluate_predictions` on a standard result frame."""
...
def print_metrics(metrics_dict: Dict[str, Any]) -> None:
    """DEPRECATED. Use :func:`print_evaluation`."""
    print(f"\n{'='*60}")
    print(f"  [EVAL] {metrics_dict['Model']} - Evaluation Results")
    print(f"{'='*60}")
    print(f"  Directional Accuracy : "
          f"{metrics_dict.get('Directional Accuracy (%)', np.nan):.2f}%")
...
def compare_models(metrics_list: List[Dict[str, Any]], save_path: Optional[str] = None):
    """DEPRECATED. Use :func:`compare_evaluations`."""
    rows = [{k: v for k, v in m.items() if k != "Confusion Matrix"} for m in metrics_list]
    df = pd.DataFrame(rows).set_index("Model")
    print(df.to_string())
    if save_path:
        df.to_csv(save_path)
